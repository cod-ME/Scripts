...
IFS=$'\n'; select opt in $alldrives "Quit"; do
    if [ "$opt" = "Quit" ]; then
        exit 0
    elif [ -n "$opt" ]; then
        chosen="$opt"
...
getmount(){
    ...
    IFS=$'\n'; select mp in $(find /mnt /media /mount /home -maxdepth 1 -type d 2>/dev/null) "Cancel"; do
        if [ "$mp" = "Cancel" ]; then
            return 1
        elif [ -n "$mp" ]; then
            break
...
case "$chosen" in
	...
	🔒*)
		...
		${TERMINAL:-kitty} -e sudo cryptsetup open "$chosen" "usb$num"
...
# 		sudo mount "/dev/mapper/usb$num" "$mp" -o uid="$(id -u)",gid="$(id -g)"
		sudo mount "/dev/mapper/usb$num" "$mp" || echo "Mount failed with exit code $?"
	...
	📱*)
		...
		sudo simple-mtpfs -o allow_other -o fsname="simple-mtpfs-$(escape "$chosen")" --device "$number" "$mp"
